...
eps_F = 10^-8*ones(1,10);
eps_X = [5*10^-4 10^-4 5*10^-5 10^-5 5*10^-6 10^-6 5*10^-7 10^-7 5*10^-8 10^-8];
...
for i = 1:length(exp_num)
    ...
    result{1} = fun_inval_maximization(Xset,type,2,eps_F(i),eps_X(i),isMax);
    ...
    tic
    %Compute the maximum of the squared norm of gradient vector for each
    result{2} = fun_inval_maximization(Xset,type,2,eps_F(i),eps_X(i),isMax);
    totalTime = totalTime + toc;

    %Compute the one-sided Lipschitz constant
    LipSumSqr = max(result{1}{1},result{2}{1}); 
    ...
    disp(' ');
    fprintf('The one-sided Lipschitz constant is: %.10f\n',LipSumSqr);
    disp(' ');
    fprintf('The overall computation time: %.3f\n',totalTime);
    
    %Store data    
    data1_tab(i,:) = {i eps_F(i) eps_X(i) LipSumSqr totalTime total_split total_split1 total_split2 total_subset gap isOpt};
    ...
    %Save data
    exp_data1.result{i} = {i, eps_F(i), eps_X(i), LipSumSqr, totalTime, result};
    save('data_experiment_OSL_X.mat','exp_data1'); 
end
