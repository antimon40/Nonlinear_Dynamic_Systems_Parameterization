%Main Program
%QIB constant computation using interval optimization: Lipschitz bound
...
%Is maximization (for upper bound) or minimization (lower bound) or
%Lipschitz bound
isMax = 2;
...
%     totalTime = totalTime + toc;
    ...
    LipSumSqr = result{1}{1}; %+ result{2}{1}; 
    ...
    disp(' ');
    fprintf('The Lipschitz bound for QIB constant is: %.10f\n',LipSumSqr);
    disp(' ');
    ...
    %Save file
    filename = 'data_experiment_table_QIB_lip_F.mat';
    save(filename,'Tab1');
    
    %Save data
    exp_data1.result{i} = {i, eps_F(i), eps_X, LipSumSqr, totalTime, result};
    save('data_experiment_QIB_lip_F.mat','exp_data1'); 
end