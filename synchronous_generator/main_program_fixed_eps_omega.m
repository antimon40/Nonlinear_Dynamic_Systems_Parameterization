...
% filename = strcat('data_simulation_one_generator_fault_reformulation.mat');
...
eps_F = [5*10^-1 10^-1 5*10^-2 10^-2 5*10^-3 10^-3 5*10^-4 10^-4 5*10^-5 10^-5]; %epsilon_h
eps_X = 10^-5*ones(1,size(eps_F,2)); %epsilon_omega
...
%Initialize data
data1_tab = cell(length(eps_X),11);
