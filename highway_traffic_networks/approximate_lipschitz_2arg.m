function lipf = approximate_lipschitz_2arg(param2,Dx,Du,lds,sample_size,param,sys,fun_mode,idx)
...
%Check input
if nargin ~= 9
    warning('Function takes 7 arguments. Try again.');
    return;
end
...
try
    if isequal(fun_mode,'p')
        fx = @traffic_fun_prop1; %(x,param,param.nonlinear_type);
    else
        fx = @traffic_fun;
    end
catch ME
...
if isequal(fun_mode,'p')
    ...
    for i = 1:sample_size 
       lipconcand = fx(Q2x(i,:)',param,param.E_N,sys); %(x,param,E_N,sys)
    ...
    %Select all pairs and compute the Lipschitz constant
    for i = 1:sample_size 
       lipconcand = fx(Q2x(i,:)',param,idx); %(x,param,E_N,sys)
       lip = max(lip,lipconcand);
    end
    lipf = lip;
end
