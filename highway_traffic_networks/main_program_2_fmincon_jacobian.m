...
for i = 1:length(Case_N_V)
    ...
    % Compute Jacobian matrix for f
    %Variables
    xsym = sym('x_s', [sys.n 1], 'real');

    %nonlinear function
    fnonlin = nonlinear_function_all(sys, param, xsym, true);
    
    %Compute Jacobian matrix
    Jmx = jacobian(fnonlin,xsym);
    name_jacobian = sprintf('jacobian_f_%d',Case_N_V_total(i));
    Jmx_handler = matlabFunction(Jmx,'File',name_jacobian,'Sparse',false,'Vars',{xsym});
    ...
    [data_all_out,data_mean_out] = function_lipschitz_computation_fmincon_jacobian(Dx,Du,sample,param,sys,Jmx_handler,sections(i));
    ...
    %Save data
    filename = num2str('database_matrix_1_globalsearch_jacobian.mat');
